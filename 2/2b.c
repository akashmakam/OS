...
typedef struct process {
    int pid, at, bt, ct, tat, wt, remaining_bt, priority;
} process;
...
process *createProcesses(int n) {
    ...
    printf("\nEnter the arrival times, burst times, and priority for the processes:\n");
    for (int i = 0; i < n; i++) {
        p[i].pid = i + 1;
        printf("\nFor Process %d:\n", i + 1);
        printf("Arrival time: ");
...
void findTimes(process *p, int n) {
    int complete = 0, et = 0, highest_priority = 9999;
    int selected = -1;
    int check = 0;
    ...
    while (complete != n) {
        for (int i = 0; i < n; i++) {
            if ((p[i].at <= et) && (p[i].priority < highest_priority) && (p[i].remaining_bt > 0)) {
        ...
        highest_priority = p[selected].priority;
